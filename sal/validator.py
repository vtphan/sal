...
class is_required(Validator):
	def __init__(self):
		super(is_required, self).__init__('is_required', None)
	...
	def sql(self, x): return '(%s != NULL)' % x

	def mongo(self, x): return {x : {'$exists' : True}}
...
class is_eq(Validator):
	...
	def sql(self, x, y): return '(%s = %s)' % (x,y)

	def mongo(self, x, y): return {x : y}
...
class is_ne(Validator):
	...
	def mongo(self, x, y): return {x : {'$ne' : y}}
...
class is_lt(Validator):
	...
	def mongo(self, x, y): return {x : {'$lt' : y}}
...
class is_le(Validator):
	...
	def mongo(self, x, y): return {x : {'$lte' : y}}
...
class is_gt(Validator):
	...
	def mongo(self, x, y): return {x : {'$gt' : y}}
...
class is_ge(Validator):
	...
	def mongo(self, x, y): return {x : {'$gte' : y}}


class is_both(Validator):
	def __init__(self):
		super(is_both, self).__init__('is_both', None)

	def __call__(self,a,b):
		return super(is_both, self).eval(a and b, a, b)

	def sql(self, x, y): return '(%s AND %s)' % (x,y)

	def mongo(self, x, y): return {'$and' : [x, y]}


class is_either(Validator):
	def __init__(self):
		super(is_either, self).__init__('is_either', None)

	def __call__(self,a,b):
		return super(is_either, self).eval(a or b, a, b)

	def sql(self, x, y): return '(%s OR %s)' % (x,y)

	def mongo(self, x, y): return {'$or' : [x, y]}


class is_not(Validator):
	def __init__(self):
		super(is_not, self).__init__('is_not', None)

	def __call__(self,a):
		return super(is_not, self).eval(not a, a, None)

	def sql(self, x): return '(NOT %s)' % x

	def mongo(self, x): return {'$not' : x}
...
class is_between(Validator):
	def __init__(self, m, n):
		self.m = m
		self.n = n
		super(is_between, self).__init__('is_between', None)
	...
	def sql(self, x, a, y): return '(%s <= %s AND %s <= %s)' % (x,a,a,y)

	def mongo(self, x, a, y): return {'$and':[{x :{'$lte':a}}, {y:{'$gte':a}}]}


class is_in(Validator):
	def __init__(self, *things):
		self.things = things
		super(is_in, self).__init__('is_in', None)
	...
	def sql(self, x, y): return '(%s in %s)' % (x,y)

	def mongo(self, x, y): return {x : {'$in' : y}}


class is_notin(Validator):
	def __init__(self, *things):
		self.things = things
		super(is_in, self).__init__('is_notin', None)

	def __call__(self, value):
		result = value in self.things
		if not result:
			self.error = 'is_notin(%s)(%s) = false' % (str(self.things), str(value))
		return result

	def sql(self, x, y): return '(NOT (%s IN %s))' % (x,y)

	def mongo(self, x, y): return {x : {'$nin' : y}}
...
class combine_validators(Validator):
	def __init__(self, validators):
		...
		super(combine_validators, self).__init__('combine_validators',None)
...
class is_value(Validator):
	def __init__(self):
		super(is_value, self).__init__('is_value', None)
...
class is_length(Validator):
	def __init__(self, m, n=None):
		self.m = m
		self.n = n
		super(is_length, self).__init__('is_legnth', None)
...
class is_email(Validator):
	...
	def __init__(self):
		super(is_email, self).__init__('is_email', None)
...
class is_url(Validator):
	...
	def __init__(self):
		super(is_url, self).__init__('is_url', None)
